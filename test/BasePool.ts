...
describe("BasePool", function () {
    ...
    before(async() => {
        ...
        depositToken = (await mockTokenFactory.deploy("DPST", "Deposit Token")).connect(account1);
        rewardToken = (await mockTokenFactory.deploy("RWRD", "Reward Token")).connect(account1);
